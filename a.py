...
from PyQt6.QtWidgets import (
    ...
    QLabel, QHBoxLayout, QPushButton, QFileDialog, QMessageBox, QDialog, QLineEdit
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        for btn in [self.btn_connect, self.btn_disconnect, self.btn_change_interface, self.btn_clear]:
        ...
        self.btn_send_drive_command = QPushButton("Send Drive Command")
        self.btn_send_drive_command.clicked.connect(self.send_drive_command)
        control_layout.addWidget(self.speed_input)
        control_layout.addWidget(self.btn_send_drive_command)
    ...
    def connect_can_interface(self):
        if self.bus is not None:
            self.show_custom_message("Warning", "Already connected.")
            return
        try:
            self.bus = can.Bus(channel=self.interface_name, interface='socketcan')
            self.timer.start(50)
            self.show_custom_message("Info", "CAN connected.")
        except Exception as e:
            self.show_custom_message("Error", f"Connection failed:\n{e}")
    ...
    def send_can_frame(self):
        try:
            ...
            self.show_custom_message("Error", f"Send failed:\n{e}")
    ...
    def send_drive_command(self):
        try:
            ...
            for m in msgs:
                ...
                time.sleep(0.05)
            #time.sleep(1)
            #self.show_custom_message("전송 완료", f"{speed:.1f} km/h 전진 명령 전송됨.")
        except Exception as e:
    ...
    def update_parsed_table(self, message):
        ...
        for name, value in parsed.items():
            ...
            for row in range(self.parsed_table.rowCount()):
                if self.parsed_table.item(row, 0).text() == name:
    ...
    def change_interface(self):
        file, ok = QFileDialog.getOpenFileName(self, "Select Interface", "/", "All Files (*)")
        if ok:
            self.interface_name = file
            self.save_config()
            self.show_custom_message("Info", "Restart app to apply interface.")
